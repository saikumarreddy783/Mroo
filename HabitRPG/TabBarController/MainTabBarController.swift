...
class MainTabBarController: UITabBarController, Themeable {
    ...
    override func viewDidLoad() {
        ...
        tabBar.items?[1].accessibilityLabel = L10n.Tasks.todos//L10n.Tasks.dailies
        tabBar.items?[2].accessibilityLabel = L10n.Tasks.dailies//L10n.Tasks.todos
    ...
    private func updateTutorialSteps(_ tutorials: [TutorialStepProtocol]) {
        for tutorial in tutorials {
            ...
            if tutorial.key == "dailies" {
                ...
                setBadgeCount(index: 1, count: tutorialDailyCount)
                // updateDailyBadge()
            }
            if tutorial.key == "todos" {
                tutorialToDoCount = tutorial.wasSeen ? 0 : 1
                setBadgeCount(index: 2, count: tutorialToDoCount)
                // updateToDoBadge()
            }
