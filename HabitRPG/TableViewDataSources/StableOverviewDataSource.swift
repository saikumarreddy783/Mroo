...
class StableOverviewDataSource<ANIMAL: AnimalProtocol>: BaseReactiveCollectionViewDataSource<StableOverviewItem> {
    ...
    override func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        if kind == UICollectionView.elementKindSectionHeader {
            ...
            label?.textColor = UIColor("#495057")
            let countLabel = view.viewWithTag(2) as? UILabel
            countLabel?.textColor = UIColor("#EA5455")
            view.viewWithTag(3)?.backgroundColor = UIColor("#EA5455").withAlphaComponent(0.2) // ThemeService.shared.theme.offsetBackgroundColor
            var ownedCount = 0
